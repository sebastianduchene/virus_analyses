...
plot(log10(combine_dat$V12.x), combine_dat$V2.x, pch = 1:10, col = 'black', ylim = c(1, 20), xlim = c(1, 4), lwd = 1.5  , ylab = 'ti/tv' , xlab = '', xaxt = 'n', cex.lab = 1.2, cex = 1.5)#, xlab = expression(paste(log[10], ' time (years)')))
...
# alpha
plot(log10(combine_dat$V12.x), combine_dat$V6.x, pch = 1:10, col = 'black', ylim = c(0, 0.2), xlim = c(1, 4), lwd = 1.5, ylab = expression(alpha), xlab = expression(paste(log[10], ' time (years)')) , cex.lab = 1.2, cex = 1.5)
